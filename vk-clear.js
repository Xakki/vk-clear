...
var fogetVk = function() {
		...
		var it = setInterval(function() {
			...
			var it2 = setInterval(function() {
				...
				for (var i = 0; i < d.length; i++) {
					if (albomDeleteItem(d[i])) {
						return true;
					}
				}
				console.log('+1', d);
				alert('All alboms delete success!');
			}, 600);

		}, 600);
	}

	var albomDeleteItem = function(item) {
	...
	var albomDeleteItem3 = function() {
		photos.deleteAlbum(cur.album, cur.albumhash);
		flatConfirm(albomsDelete);
	}

	var albomDeleteItem2 = function(pool) {
		if (!pool.length) {
			Photoview.hide(0);
			albomsDelete();
			return ;
		...
		clickBy(geByTag('a', cur)[0], function(){
			...
			setTimeout(function(){
				albomDeleteItem2(pool);
			}, 1100);
		});
		
	}

	var albomDeleteItem1 = function() {
